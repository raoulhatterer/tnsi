...
- Les scripts sont de courts programmes généralement faciles à construire. *Bash* offre un service de gestion de fiux, c’est-à-dire qu’il permet que le résultat d’un script (la sortie) soit transmis à un autre script (l’entrée). De cette façon, les scripts peuvent être « chaînés », chacun effectuant une seule tâche bien délimitée.
